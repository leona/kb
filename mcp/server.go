...
func Serve() error {
	...
	s.AddTool(mcp.NewTool("kb_write",
		mcp.WithDescription("Create or update a file in the knowledge base. IMPORTANT: Always use kb_draft first to show the user a preview before calling this tool. Path must be under shared/ or projects/. Parent directories are created automatically. Changes are auto-committed unless no_commit is true. NOTE: Do not include <!-- KB:REFS -->, <!-- KB:INLINE -->, or <!-- /KB:INLINE --> comment blocks in your content — these are auto-generated and will be stripped and regenerated automatically."),
		mcp.WithString("path", mcp.Required(), mcp.Description("Relative path within KB (e.g., 'shared/my-shared-doc/api-docs.md' or 'projects/myapp/context.md').")),
...
// isContextMD returns true if the clean path points to a project's context.md.
func isContextMD(cleanPath string) bool {
	parts := strings.SplitN(cleanPath, "/", 3)
	return len(parts) >= 3 && parts[0] == "projects" && parts[2] == "context.md"
}
...
func handleDraft(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if isContextMD(cleanPath) {
		content = project.StripGeneratedBlocks(content)
	}
...
func handleWrite(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Strip auto-generated KB:REFS and KB:INLINE blocks from context.md writes.
	// Agents often copy these markers from kb_context output; they will be
	// regenerated by UpdateRefsInventory after writing.
	if isContextMD(cleanPath) {
		content = project.StripGeneratedBlocks(content)
	}
